...
$_valid = $_smarty_tpl->decodeProperties(array (
  ...
  'nocache_hash' => '12259968815996a95392fc19-54814946',
  'function' => 
  array (
  ),
...
<script>
    ...
    $(function(){
        ...
        /*       $.ajax({
                   type: "GET",
                   url: "http://core.app/avgleinit",
                   dataType: "json",
                   success: function(data){
                       if(data.status == undefined || data.status !== 0){
                           var msg = data.msg == undefined ? '视频目录获取失败,请刷新重试' : data.msg;
                           alert(msg);
                       }
                       ...
                       var list_num = 0;
                       $(data.data.categories).each(function (index, item) {
                           if(list_num == 3){
                               list_num = 0;
                           }
                           $('#categories' + list_num).append('<li><a href="archive.html">' + item.name + '</a></li>');

                           list_num++;
                       });

                       $(data.data.hotVideos).each(function (index, item) {
