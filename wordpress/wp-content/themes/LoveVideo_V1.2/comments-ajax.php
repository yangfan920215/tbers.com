...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
        err( __('签名或邮箱未输入')); // 將 wp_die 改為錯誤提示
	elseif ( !is_email($comment_author_email))
        err(__('请输入您的邮箱地址')); // 將 wp_die 改為錯誤提示
}

if ( '' == $comment_content )
    err(__('请输入评论内容')); // 將 wp_die 改為錯誤提示
...
?>
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<div class="comment-author vcard">
			<img src="<?php echo bloginfo('template_directory'),'/assets/images/gravatar.jpg' ?>" alt="" class="avatar" />
			<?php printf( __( '<strong>%s</strong>'), get_comment_author_link() ); ?>:<?php edit_comment_link( __( '编辑' ), ' ' ); ?>
		</div>
