...
class Single_Post_Template_Plugin {
	...
	function add_metabox() {
		...
		if ( $this->get_post_templates() )
			add_meta_box( 'pt_post_templates', __( '页面属性', 'genesis' ), array( $this, 'metabox' ), 'post', 'normal', 'high' );
	...
	function metabox( $post ) {
		...
		<label class="hidden" for="post_template"><?php  _e( '文章页模板', 'genesis' ); ?></label><br />
		...
		<p><?php _e( '选择适合本篇文章的模板。', 'genesis' ); ?></p>
		<?php
